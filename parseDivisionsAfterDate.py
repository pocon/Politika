		...
		for division in procHansard.findAll("division"): # Parse each <division>
			...
			for member in members:
				if member.parent.name=="memberlist":
					if member["vote"]=="aye": pass#member["id"] will give the id for the aye vote
					else:pass# As above. Also: assuming all votes are either "aye" or "no" and there are no errors in the xml
				elif member.parent.name=="pairs": # process the pairs
					pairmembers=member.parent.findAll("member")
					# pairmembers[0]["id"] and pairmembers[1]["id"] give you the id for each member
			...
			if division.findPreviousSibling().name=="speech": # assuming the reason for division is given in the <p> childs of the first <speech> tag before the <division> tag
				...
				for pTag in division.findPreviousSibling().findAll("p"):
					...
					# variable reason now contains a string representing the apparent reason for the division
			# You can get the url of the division in the official hansard from aph with: str(division["url"])
			# date gives the date on which the division occurred as a date object
	else:
